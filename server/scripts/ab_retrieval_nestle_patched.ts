/**
 * Stage 2 A/B retrieval harness (Nestle, framework 3).
...
const existingCorpus: Array<{ url: string; title: string; text: string }> = JSON.parse(readFileSync(`${HOME}/ab_nestle_corpus.json`, "utf8"));
const newDocs: Array<{ url: string; title: string; text: string }> = JSON.parse(readFileSync(`${HOME}/ab_nestle_new_docs.json`, "utf8"));
...
writeFileSync(`${HOME}/ab_stage2_retrieval_nestle_patched.json`, JSON.stringify({
  meta: {
    deterministicTerms,
...
console.error("\nSAVED ab_stage2_retrieval_nestle_patched.json");
