...
- ACCEPTANCE-FIRST: on EVERY turn that presents a proposal or asks a question, the FIRST \`[[option:...]]\` line MUST be an acceptance option that means "proceed with exactly what I just proposed" — use a clear accept label such as [[option:Accept the proposal]], [[option:Accept all as proposed]], or [[option:Yes, proceed]]. The UI promotes this first chip to a prominent primary button, so it must always be present and must always be the accept action. List the more specific alternative choices AFTER it.
